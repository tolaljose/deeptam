from ..python.deeptam_tracker.tracker import Tracker
from ..python.deeptam_tracker.evaluation.rgbd_sequence import RGBDSequence
from ..python.deeptam_tracker.evaluation.metrics import rgbd_rpe
from ..python.deeptam_tracker.utils.vis_utils import convert_between_c2w_w2c, convert_array_to_colorimg
...
def track_rgbd_sequence(checkpoint, datadir, tracking_module_path, visualization):
    ...
    pose0_gt = frame['pose']
    tracker.clear()
    tracker.set_init_pose(
        pose0_gt)  # this step can be left out if gt_poses is aligned such that it starts from identity pose
...
def main():
    ...
    parser.add_argument('--data_dir',
                        ...
                        default=None)
    parser.add_argument('--disable_vis', help='disable the frame-by-frame visualization for speed-up',
                        action='store_true')
